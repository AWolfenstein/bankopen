...
export function validCardRUSFunc(Flag, callback) {
  ...
  if (Flag !== "RUS") {
    return callback(<ErrorMsng value={i18n.t('TrasferCardErorForRus')}></ErrorMsng>)
  }
}
...
export function validAmountFunc(Flag, value, callback) {
  ...
  if (Flag == "RU") {
    if (amount <= 140000) {
      ...
      return callback(<ErrorMsng value={i18n.t('InvalidAmmountRus')}></ErrorMsng>)
    }
  }
  else {
    if (amount <= 100000) {
...
export function validAmountminFunc(value, callback) {
  ...
  if (amount >= 10) {
    ...
    return callback(<ErrorMsng value={i18n.t('InvalidAmmountmin')}></ErrorMsng>)
  }
}
...
let masterc= ["CUB","IRN","SDN","PRK"];
let visac =["AZE","ARM","BLR","BGR","GRC","GEO","KAZ","KGZ","LVA","LTU","MDA","ROU","SVK","СВН","TJK","TKM","UKR","UZB","CZE","RUS"]
  if (paySys === "visa") {
    console.log(Flag)
